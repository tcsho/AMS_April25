...
public partial class PresentationLayer_MasterPage : System.Web.UI.MasterPage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            getHeadOfDept(Session["EmployeeCode"].ToString());
        ...
        foreach (DataRow subMenu in _dtmenu.Rows)
        {
            if (Convert.ToBoolean(subMenu["isAllow"]))
            {
                ...
                else
                {
                    ...
                    {
                        if (subMenu["Menu_ID"].ToString() == "74")
                        {
                            if (Session["UserType"].ToString() == "17" || Session["UserType"].ToString() == "20")
                            {
                                if (HoDCode == string.Empty)
                                {
                                    innerHtml += "<a href=\"" + Page.ResolveUrl(subMenu["PagePath"].ToString()) + "\" id=\"_le098\">" +
                                                 subMenu["MenuText"].ToString() + "</a>";
                                }
                                else
                                {
                                    innerHtml += "<a href=\"" + Page.ResolveUrl(subMenu["PagePath"].ToString()) + "\">" +
                                                 subMenu["MenuText"].ToString() + "</a>";
                                }
                            }
                        }
                        else if (subMenu["Menu_ID"].ToString() == "75")
                        {
                            if (Session["UserType"].ToString() == "17" || Session["UserType"].ToString() == "18" || Session["UserType"].ToString() == "20" || Session["UserType"].ToString() == "21" || Session["UserType"].ToString() == "23" || Session["UserType"].ToString() == "24")
                            {
                                ...
                                {
                                    innerHtml += "<a href=\"" + Page.ResolveUrl(subMenu["PagePath"].ToString()) + "\" id=\"_le099\">" +
                                                 subMenu["MenuText"].ToString() + "</a>";
                                }
                                else
                                {
    ...
    private void getHeadOfDept(string empcode)
    {
        string connectionString = ConfigurationManager.ConnectionStrings["tcs_invConnectionString"].ConnectionString;
        string selectQuery = "SELECT distinct HoDCode FROM HoDInfo4EmpLeaveEncashment WHERE HoDCode = " + empcode;
            ...
            try
            {
                ...
                using (SqlDataReader reader = command.ExecuteReader())
                {
                    if (reader.Read())
                    {
                        ...
                        HoDCode =  reader["HoDCode"].ToString().Trim();
                    }
                    else
                    {
                        HoDCode = string.Empty;
                    }
                }
            }
            catch (Exception ex)
    ...
    private void getEmpIfClearenceFormAvailable(string empcode)
    {
        string connectionString = ConfigurationManager.ConnectionStrings["tcs_invConnectionString"].ConnectionString;
        string selectQuery = "SELECT distinct EmployeeCode FROM EmployeeClearenceRights WHERE Status_Id = 1 AND EmployeeCode = " + empcode;
        ...
        using (SqlConnection connection = new SqlConnection(connectionString))
        {
            ...
            try
            {
                ...
                using (SqlDataReader reader = command.ExecuteReader())
                {
                    ...
                    {
                        // Access the data (replace "ColumnName" with actual column names)
                        EmployeeNo = reader["EmployeeCode"].ToString().Trim();
                    }
                    else
                    {
